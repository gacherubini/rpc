...
func main() {
	if len(os.Args) < 3 {
		...
		fmt.Println("Operações disponíveis: abrir, fechar, depositar, sacar, saldo")
		return
	}
	...
	case "abrir":
		if len(os.Args) < 4 {
			fmt.Println("Uso para abrir conta:", os.Args[0], "<maquina> abrir <saldo_inicial>")
			return
		}
		var idConta int
		saldoInicial := parseFloat(os.Args[3])
		err = client.Call("ServicoContas.AbrirConta", saldoInicial, &idConta)
		if err != nil {
			fmt.Println("Erro ao abrir conta:", err)
		} else {
			fmt.Printf("Conta criada com sucesso! ID da conta: %d\n", idConta)
		}

	case "fechar":
		if len(os.Args) < 4 {
			fmt.Println("Uso para fechar conta:", os.Args[0], "<maquina> fechar <id_conta>")
			return
		}
		idConta := parseInt(os.Args[3])
		var resultado bool
		err = client.Call("ServicoContas.FecharConta", idConta, &resultado)
		if err != nil || !resultado {
			fmt.Printf("Erro ao fechar conta ID %d: %v\n", idConta, err)
		} else {
			fmt.Printf("Conta ID %d fechada com sucesso!\n", idConta)
		}
